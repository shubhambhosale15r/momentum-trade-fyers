...
def process_symbol(t, start, end):
    try:
        ...
        if (pd.notna(vol) and vol > 0) and (r3 > 0  and r2 > 0 and r1 > 0) and (r3 > r2 > r1):
            mom = (((0.5*r3 if pd.notna(r3) else 0)) +
                   ((0.3*r2 if pd.notna(r2) else 0)) +
                   ((0.2*r1 if pd.notna(r1) else 0))) / vol
